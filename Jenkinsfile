...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'CI_NOBUILD',
                     defaultValue: false,
                     description: 'Do not build daos.  Use for PR and special branches only')
        string(name: 'CI_RPM_TEST_VERSION',
               defaultValue: '1.3.101-1',
               description: 'Prebuilt RPMs to use instead of building DAOS. (Special CI use)')
        booleanParam(name: 'CI_ALLOW_UNSTABLE_TEST',
                     defaultValue: false,
                     description: 'Continue testing if a previous stage is Unstable')
        booleanParam(name: 'CI_UNIT_TEST',
                     defaultValue: true,
                     description: 'Run the Unit CentOS 7 CI tests')
        booleanParam(name: 'CI_UNIT_TEST_MEMCHECK',
                     defaultValue: true,
                     description: 'Run the Unit Memcheck CentOS 7 CI tests')
